function migrateStyleFile(stylesFileContent, importStatements, componentName) {
  let returnStyleFileContent = ''
  try {
    ...
    returnStyleFileContent = defaultExportStyleSheet.replace(
      styleImportRegex,
      (match) => {
        let str = ''
  ...
  try {
    ...
    if (styledReactComponents?.length > 0) {
  ...
  return { finalStyleFile: returnStyleFileContent, styledComponentMap }
}
